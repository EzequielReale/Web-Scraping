...
onBeforeMount(() => {
    ...
    getErrorsById(route.params.id).then(
        result => webErrors.value = result
    );
});
...
.error-content {
  max-width: mx-auto; /* Adjust the max-width according to your design */
  word-break: break-word; /* Allow long words to break onto the next line */
  white-space: pre-wrap; /* Wrap the text and preserve line breaks */
}
